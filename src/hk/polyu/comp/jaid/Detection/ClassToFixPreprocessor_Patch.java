...
public class ClassToFixPreprocessor_Patch {
    private final JavaProject project;
    private final Config config;

    public ClassToFixPreprocessor_Patch(JavaProject project, Config config) {
...
//获取b的根节点（在ast中）
        ...
        //获取中间文件路径
        project.registerIntermediatePatchSourceFilePaths(compilationUnit);

        // Rewrite method to fix，反正这一步就是写jaid_output.formatted_src.af_test.MyList.java这个文件
        Document document = new Document(FileUtil.getFileContent(project.getPatchSourceFileToFix(), StandardCharsets.UTF_8));
...
//            writeFile(project.getFormattedSourceFileToFix(), newContent);
            writeFile(project.getPatchFormattedSourceFileToFix(), newContent);
    ...
    private MethodDeclaration getMethodDeclarationToFix() {
        String[] files = new String[project.getPatchSourceFiles().size()];
        project.getPatchSourceFiles().stream().map(x -> x.toString()).collect(Collectors.toList()).toArray(files);
